...
if($_GET["optn"]==1){
    ...
    if ($msConn) {
        ...
        if ($username != $_SESSION["username"]) $querySql = "UPDATE `corriere` SET `username` = '$username', `telefono` = '$tel' WHERE `id` = '" . $_SESSION["id"] . "'";
        else $querySql = "UPDATE `corriere` SET `telefono` = '$tel' WHERE `id` = '" . $_SESSION['id'] . "'";
    ...
    if ($msConn) {
        ...
        $querySql = "SELECT `password` FROM  `corriere` WHERE `id` = '" . $_SESSION['id'] . "' AND `password` = '$old'";
        ...
        try{
            ...
            $querySql = "UPDATE `corriere` SET `password` = '$new' WHERE `id` = '" . $_SESSION['id'] . "'";
            $queryRes = mysqli_query($msConn, $querySql);   
    ...
    if ($msConn) {
        ...
        $querySql = "UPDATE `corriere` SET `copertura` = '". $copertura ."' WHERE `id` = '" . $_SESSION['id'] . "'";
        ...
        try{
            ...
            if(!$queryRes) throw new Exception('err1');
            else header("Location: //".$_SERVER['SERVER_NAME'] . "/msmr/auth/settings.php?optn=3&err=0");

             
        }catch(Exception $exc){
            echo $exc->getMessage();
            //header("Location: //".$_SERVER['SERVER_NAME'] . "/msmr/auth/settings.php?optn=3&err=1");
        }
